...
This section of the manual will provide an overview of the steps taken in the ImageJ script,starting with the inputs required for the script to function, followed by a walkthrough of how each marker is analysed.

## Specifying Parameters

When the ImageJ script is ran, the following dialog boxes will appear one at a time, with the latter dialog boxes changing depending on the initial inputs. Here, we will outline each input required.

### Dialog Box (1/3)

![Dialog Box (1/3)](images/Screenshot%202022-08-15%20at%2013.39.49.png){width="400"}
...
Specify image directory: Specify the image directory here.
...
Use Gaussian and Watershed on ROI: Select "Yes" or "No". If "Yes", this function blurs the ROIs before using a Watershed transform to segregate them. This is useful when the ROI signal is fragmented and is not densely populated within the region.
...
Here I present a comparison between the script and blind manual counts. The aim of the code was to imitate manual counts while speeding up the time taken to analyse collections of images. Images IDs have been removed from the plot for privacy purposes. We are happy with the results of these counts and aim to add more manual count data to the comparison in the near future.

![](images/Iba1_counts.png)
...
The default threshold is again set to 'Otsu' which we recommend due to its balance between removing noise and retaining useful signal. Find here a few example thresholding montages.
...
The ImageJ script output consists of a results table for every case. These results tables will be collated and read by the R script.
